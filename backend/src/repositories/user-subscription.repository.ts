...
import { IUserSubscriptionRepository, UserSubscriptionWithPlan, ISyncTransactionContext, SubscriptionStatus } from "@/interfaces/repositories/user-subscription.repository.interface";
...
export class UserSubscriptionRepository implements IUserSubscriptionRepository {
    ...
    */
   async executeSyncTransaction<T>(
      userId: number,
      operation: (ctx: ISyncTransactionContext) => Promise<T>
   ): Promise<T> {
            ...
            writeAuditLog: async (params) => {
               await tx.userSubscriptionLog.create({
                  data: {
                     ...
                     // Minimal default values since these aren't from webhooks
                     eventType: "SYNC",
                     eventId: `sync_${Date.now()}_${Math.random().toString(36).substring(7)}`,
                  },
               });
            }
         };
