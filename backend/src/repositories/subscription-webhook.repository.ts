...
import { ISubscriptionWebhookRepository, ProcessWebhookParams } from "@/interfaces/repositories/subscription-webhook.repository.interface";
...
const DOWNGRADE_EVENT_TYPES: RevenueCatWebhookEvent[] = [RevenueCatWebhookEvent.EXPIRATION, RevenueCatWebhookEvent.REFUND, RevenueCatWebhookEvent.CANCELLATION, RevenueCatWebhookEvent.BILLING_ISSUE];
...
export class SubscriptionWebhookRepository implements ISubscriptionWebhookRepository {
   async processWebhookEvent(params: ProcessWebhookParams): Promise<boolean> {
      const { userId, event, targetPlanId, freePlanId, freePlanDurationDays, defaultSubscriptionDurationDays, buildFeatureFullPayload, buildFeatureLimitsOnlyPayload, shouldApplyFeaturePayload = true } = params;
            ...
            switch (event.type) {
               ...
               case RevenueCatWebhookEvent.PRODUCT_CHANGE: {
                  ...
                  if (targetPlan && shouldApplyFeaturePayload) {
                     const featurePayload = isInitialPurchase ? buildFeatureFullPayload(targetPlan) : buildFeatureLimitsOnlyPayload(targetPlan);
               ...
               case RevenueCatWebhookEvent.EXPIRATION: {
                  ...
                  const expirationUpdate = await tx.userSubscription.updateMany({
                     where: whereClause,
                     data: {
                        status: "GRACE_PERIOD",
                        expiredAt: new Date(),
                  ...
                  if (expirationUpdate.count === 0 && currentSubscription) {
                     await tx.userSubscription.update({
                        where: { id: currentSubscription.id },
                        data: {
                           status: "GRACE_PERIOD",
               ...
               case RevenueCatWebhookEvent.REFUND: {
                  ...
                  const refundUpdate = await tx.userSubscription.updateMany({
                     where: whereClause,
                     data: {
                        status: "EXPIRED",
                        refundedAt: new Date(),
                  ...
                  if (refundUpdate.count === 0 && currentSubscription) {
                     await tx.userSubscription.update({
                        where: { id: currentSubscription.id },
                        data: {
                           status: "EXPIRED",
