...
class _WebProfileDetailScreenState extends State<WebProfileDetailScreen>
  ...
  Widget build(BuildContext context) {
    if (!hasApiData) {
      return Scaffold(
        ...
        body: const Center(child: SizedBox(width: 800, child: ProfileSkeleton())),
      );
    }
    return Scaffold(
      backgroundColor: Theme.of(context).canvasColor,
  ...
  Widget _buildBody() {
    final profileMap = resolvedProfile;

    final isLoading = _isPassing || _isInterested;
                ...
                children: [
                  ...
                  Expanded(
                    ...
                    child: _WebPhotoGallery(
                      ...
                      onPass: () => _handlePass(profileMap['id'] as int?),
                      onLike: () => _handleLike(profileMap['id'] as int?),
                      isLoading: isLoading,
                    ),
                  ),
                  // Right Side - Details
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: _WebProfileDetails(
                        profileMap: profileMap,
                        onReportPressed: () => ReportUserDialog.show(context, profileMap),
                        onBlockPressed: () => _showBlockConfirmation(profileMap),
                      ),
                    ),
                  ),
                ],
  ...
  Future<void> _handlePass(int? profileId) async {
    ...
    try {
      await context.read<MatchProvider>().swipeLeft(targetProfileId: profileId);
      if (!mounted) return;
      if (context.canPop()) {
        context.pop();
      } else {
      ...
      if (e is DioException && e.response?.statusCode == 402) {
        FeatureExhaustedModal.show(context, featureType: 'Skip');
      } else {
  ...
  Future<void> _handleLike(int? profileId) async {
    ...
    try {
      await context.read<MatchProvider>().swipeRight(targetProfileId: profileId);
      if (!mounted) return;
      if (context.canPop()) {
        context.pop();
      } else {
